/**
 * Custom Feature: active-learning
 * Active learning item selection
 *
 * POST /api/ai/active-learning
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_ACTIVE_LEARNING_KEY
...
const FEATURE_KEY = process.env.FEATURE_ACTIVE_LEARNING_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Active learning item selection.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDataLabelingPlatform - active-learning',
    },
    body: JSON.stringify({
...
router.post('/active-learning', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_ACTIVE_LEARNING_KEY');
    const ai = await callLLM({ feature: 'active-learning', goal: 'Active learning item selection', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['active-learning', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[active-learning] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'active-learning',
      endpoint: '/api/ai/active-learning',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[active-learning] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/active-learning/health', (req, res) => {
  res.json({
    feature: 'active-learning',
    endpoint: '/api/ai/active-learning',
